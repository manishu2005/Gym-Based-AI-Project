...
class VoicePipeline:
    ...
    def _find_form_issue(self, excercise, metrics):
        if "issue" in metrics:
        ...
        if excercise == "Squats":
            ...
            if depth == "TOO HIGH":
                return "The user's squat is not deep enough - knees are not bedning sufficiently."
            ...
            if swing == "SWINGING":
                return "The user's body is swinging their torso during the curl -keep the body straight."
            ...
            if back_arch == "Excessive Arch":
                return "The user's body is arching their lower back excessively during the press"
            
            
            if back_arch == "Slight Arch":
                return "The user's body is slight back arch detected - encourage the user to brace their core."
    ...
    def process_event(self, event, excercise, metrics):
        issue = self._find_form_issue(excercise, metrics)
        ...
        print("Voice size:", len(voice) if voice else 0)
